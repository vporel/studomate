...
import { STEP_HANDLE_SOURCE_SUCCESSOR, STEP_HANDLE_TARGET_PREDECESSOR } from "@/schemas/grafcet/step.schema";
import {
...
function transitionToReferralSource(id: string, transitionId: string, referralSourceId: string) {
	return new ConnectionBuilder()
		.id(id)
		.source("transition", transitionId, TRANSITION_HANDLE_SOURCE_SUCCESSOR)
		.target("step-referral-source", referralSourceId, STEP_REFERRAL_SOURCE_HANDLE_TARGET_PREDECESSOR)
		.build();
}
...
describe("StepReferralTargetAnalyser", () => {
	...
	describe("analyseInContext", () => {
		 ...
		 */
		function buildReferralPair(referralTargetId: string, sourceStepNumber: number) {
			const referralTarget = new StepReferralTargetBuilder()
				.id(referralTargetId)
				.sourceStepNumber(sourceStepNumber)
				.build();
			const downstreamStep = new StepBuilder().id("step-downstream").number(9).build();
			...
			const referralToStep = new ConnectionBuilder()
				...
				.source("step-referral-target", referralTargetId, STEP_REFERRAL_TARGET_HANDLE_SOURCE_SUCCESSOR)
		...
		it("signale une erreur si le tenant n'a aucun prédécesseur", () => {
			const { referralTarget, downstreamStep, referralSource, referralToStep } = buildReferralPair(
				"referral-err",
				1,
			);
			...
			const issues = analyser.analyseInContext(referralTarget, grafcet, []);
