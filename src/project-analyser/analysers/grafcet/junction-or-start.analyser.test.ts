...
describe("JunctionOrStartAnalyser", () => {
	...
	describe("analyseInContext", () => {
		...
		it("detects a branch not connected to a transition", () => {
			...
			const typeIssue = issues.find((i) => i.code === "JUNCTION_OR_START_BRANCH_NOT_TRANSITION");
			expect(typeIssue).toBeDefined();
			expect(typeIssue?.severity).toBe("error");
		});

		it("detects fewer than two branches", () => {
			const junction = new JunctionOrStartBuilder().id("junction-1").nBranches(0).build();
			const grafcet = new GrafcetBuilder().id("grafcet-1").addJunctionOrStart(junction).build();
		...
		it("detects two branches with identical receptivities", () => {
			...
			const branchIds = junction.data.branchesOrder;
			const trans1 = new TransitionBuilder().id("trans-2").expression("Btn1").build();
			const trans2 = new TransitionBuilder().id("trans-3").expression("Btn1").build();
			const c1 = new ConnectionBuilder()
				.id("c1")
				.source("step", "step-0", "source:successor")
				.target("junction-or-start", "junction-1", "pivot")
			...
			const exclusivityIssue = issues.find((i) => i.code === "JUNCTION_OR_START_BRANCHES_NOT_EXCLUSIVE");
		...
		it("detects a branch with a receptivity that is always true", () => {
			const junction = new JunctionOrStartBuilder().id("junction-1").nBranches(2).build();
			const branchIds = junction.data.branchesOrder;
			const trans1 = new TransitionBuilder().id("trans-2").expression("VRAI").build();
			const trans2 = new TransitionBuilder().id("trans-3").expression("Btn1").build();
			const c1 = new ConnectionBuilder()
				.id("c1")
				.source("step", "step-0", "source:successor")
