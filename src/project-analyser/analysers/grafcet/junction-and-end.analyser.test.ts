...
describe("JunctionAndEndAnalyser", () => {
	...
	describe("analyseInContext", () => {
		...
		it("returns no issues for a parallélisme imbriqué (nested AND convergence)", () => {
			...
			expect(
				analyser.analyseInContext(jaeOuter, grafcet, analyserEnvironment()),
			).toHaveLength(0);
			expect(
				analyser.analyseInContext(jaeInner, grafcet, analyserEnvironment()),
			).toHaveLength(0);
		});

		it("detects AND convergence with no junction-and-start reachable", () => {
			const { junction, grafcet } =
				buildAndTopologyWithNoOppositeJunction("end");
		...
		it("detects AND convergence branches coming from different junction-and-starts", () => {
			const { junction, grafcet } =
				buildAndTopologyWithMismatchedOppositeJunctions("end");
		...
		it("detects mismatched branch count between JAE and JAS", () => {
			const { junction, grafcet } =
				buildAndTopologyWithMismatchedBranchCount("end");
