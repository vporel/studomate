...
describe("JunctionAndStartAnalyser", () => {
	...
	describe("analyseInContext", () => {
		...
		it("returns no issues for a parallélisme imbriqué (nested AND divergence)", () => {
			...
			expect(analyser.analyseInContext(jasOuter, grafcet, [])).toHaveLength(0);
			expect(analyser.analyseInContext(jasInner, grafcet, [])).toHaveLength(0);
		});

		it("detects AND divergence with no junction-and-end reachable", () => {
			const { junction, grafcet } = buildAndTopologyWithNoOppositeJunction("start");
		...
		it("detects AND divergence branches reaching different junction-and-ends", () => {
			const { junction, grafcet } = buildAndTopologyWithMismatchedOppositeJunctions("start");
		...
		it("detects mismatched branch count between JAS and JAE", () => {
			const { junction, grafcet } = buildAndTopologyWithMismatchedBranchCount("start");
