...
describe("LadderAnalyser", () => {
	...
	describe("buildTimerExposedVariables/buildCounterExposedVariables (via generateVariables)", () => {
		it("ignore un bloc tempo dont le nom est invalide, sans lever", () => {
			const rail = createRailTerminalElement(0);
			const block = createTimerBlockElement({ name: "1Tempo", timerType: "TON", pt: "T#5s" }, 0, 1);
			const ladder = new Ladder("l1", "L", [createSectionWith([rail, block], wireInSeries([rail, block]))]);

			expect(() => ladderAnalyser.generateVariables(ladder)).not.toThrow();
			const generatedVariables = ladderAnalyser.generateVariables(ladder);
			expect(generatedVariables.some((v) => v.mnemonic.startsWith("1Tempo."))).toBe(false);
		});
		...
		it("ignore un bloc compteur dont le nom est invalide, sans lever", () => {
			...
			expect(() => ladderAnalyser.generateVariables(ladder)).not.toThrow();
			const generatedVariables = ladderAnalyser.generateVariables(ladder);
			expect(generatedVariables.some((v) => v.mnemonic.startsWith("1Compteur."))).toBe(false);
		});
	});
	...
	describe("checkConnectionColumnOrder (via analyse)", () => {
		it("ne signale rien quand les connexions vont bien vers une colonne ultérieure ou égale", () => {
			...
			expect(issues.map((i) => i.code)).not.toContain("LADDER_CONNECTION_INVALID_ORDER");
		});
		...
		it("signale LADDER_CONNECTION_INVALID_ORDER quand une connexion pointe vers une colonne antérieure", () => {
			...
			expect(issues.map((i) => i.code)).toContain("LADDER_CONNECTION_INVALID_ORDER");
		});
	});

	describe("checkBlockNameConflicts", () => {
		it("ne signale rien quand tous les noms de blocs sont distincts", () => {
			const timer = createTimerBlockElement({ name: "Tempo1", timerType: "TON", pt: "T#5s" }, 0, 0);
			const counter = createCounterBlockElement(
				{ name: "Compteur1", counterType: "CTU", control: "R", pv: "5" },
				0,
				0,
			);
			const ladder = new Ladder("l1", "L", [createSectionWith([timer, counter], [])]);
		...
		it("signale BLOCK_NAME_DUPLICATE quand deux blocs partagent le même nom", () => {
			const timer1 = createTimerBlockElement({ name: "T1", timerType: "TON", pt: "T#5s" }, 0, 0);
			const timer2 = createTimerBlockElement({ name: "T1", timerType: "TON", pt: "T#5s" }, 0, 1);
			const ladder = new Ladder("l1", "L", [createSectionWith([timer1, timer2], [])]);
			const project = ProjectFactory.createEmpty();
			project.addProgram(ladder);
		...
		it("signale BLOCK_NAME_DUPLICATE entre un timer et un compteur de même nom", () => {
			const timer = createTimerBlockElement({ name: "X1", timerType: "TON", pt: "T#5s" }, 0, 0);
			const counter = createCounterBlockElement({ name: "X1", counterType: "CTU", control: "R", pv: "5" }, 0, 1);
			const ladder = new Ladder("l1", "L", [createSectionWith([timer, counter], [])]);
			const project = ProjectFactory.createEmpty();
			project.addProgram(ladder);
		...
		it("signale BLOCK_NAME_VARIABLE_CONFLICT quand un nom de bloc collisionne avec une variable existante", () => {
			const timer = createTimerBlockElement({ name: "A", timerType: "TON", pt: "T#5s" }, 0, 0);
			const ladder = new Ladder("l1", "L", [createSectionWith([timer], [])]);
			const project = ProjectFactory.createWithVariables([VariableFactory.createLogicInput("A")]);
			project.addProgram(ladder);

			const issues = LadderAnalyser.checkBlockNameConflicts(project);

			expect(issues.map((i) => i.code)).toEqual(["BLOCK_NAME_VARIABLE_CONFLICT"]);
		});

		it("ignore les blocs sans nom (déjà signalés ailleurs)", () => {
			const timer1 = createTimerBlockElement({ name: "", timerType: "TON", pt: "T#5s" }, 0, 0);
			const timer2 = createTimerBlockElement({ name: "", timerType: "TON", pt: "T#5s" }, 0, 1);
			const ladder = new Ladder("l1", "L", [createSectionWith([timer1, timer2], [])]);
			const project = ProjectFactory.createEmpty();
			project.addProgram(ladder);
