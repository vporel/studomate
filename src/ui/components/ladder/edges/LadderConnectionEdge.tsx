...
export type LadderConnectionEdgeType = Edge<LadderConnectionEdgeData> & {
	type: "ladder-connection";
};
 ...
 * côté nœuds. */
const quarterColToPx = (quarterCol: number) =>
	POWER_RAIL_OFFSET + quarterCol * QUARTER_CELL_WIDTH;
...
function buildVertices(
	...
	if (points.length === 0)
		return getConnectionLinePoints(sourceX, sourceY, targetX, targetY);
	const bend: Vertex[] = points.map(([, quarterCol], i) => [
		quarterColToPx(quarterCol),
		i === 0 ? sourceY : targetY,
	]);
...
function findVerticalSegment(
	vertices: Vertex[],
): { x: number; yA: number; yB: number } | null {
...
const LadderConnectionEdge = ({
	...
	const energized = useProjectStore(
		useShallow((state) => {
			if (!pageVisible || !ladder || state.mode !== ProjectMode.SIMULATION)
				return false;
			return computeEnergizedEdges(ladder, state.simulationVariablesStates).has(
				id,
			);
		}),
	);
	...
	const renderedVertices =
		liveQuarterCol !== null && verticalSegment
			? vertices.map(
					([x, y]) =>
						(x === verticalSegment.x
							? [quarterColToPx(liveQuarterCol), y]
							: [x, y]) as Vertex,
				)
			: vertices;
	...
	const handlePointerMove = useCallback(
		(e: React.PointerEvent<SVGLineElement>) => {
			...
			const snappedQuarterCol = Math.round(
				(x - POWER_RAIL_OFFSET) / QUARTER_CELL_WIDTH,
			);
	...
	const handlePointerUp = useCallback(
		(e: React.PointerEvent<SVGLineElement>) => {
			...
			const newPoints: [number, number][] = points.map(([row]) => [
				row,
				finalQuarterCol,
			]);
	...
	return (
		<Box
			...
			sx={{
				...
				"&:hover .ladder-edge-point": {
					opacity: mode === ProjectMode.DESIGN ? 1 : 0,
				},
			}}
		>
			<path
				...
				style={
					energized
						? { stroke: th.palette.energized.main, strokeWidth: 2 }
						: undefined
				}
			/>
			{interactionWidth ? (
