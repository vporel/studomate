...
describe("useExplorerContextMenu", () => {
	it("is hidden by default, targeting the pane", () => {
		const { result } = renderHook(() => useExplorerContextMenu(createRef({ left: 0, top: 0 })))
	...
	it("opens the context menu at a position relative to the explorer", () => {
		...
		act(() => {
			result.current.openContextMenu({ clientX: 50, clientY: 70 } as any, element)
		})
	...
	it("closes the context menu", () => {
		...
		act(() => {
			result.current.openContextMenu({ clientX: 5, clientY: 5 } as any, { type: "pane" })
		})
		act(() => {
