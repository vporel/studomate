...
export default function useHmiMarqueeSelect(
	...
	const dragState = useRef<{ startX: number; startY: number; additiveBase: string[] } | null>(null);
	...
	return (e: ReactMouseEvent, currentSelection: string[]) => {
		...
		dragState.current = { startX: start.x, startY: start.y, additiveBase: additive ? currentSelection : [] };

		const rectFrom = (current: { x: number; y: number }, drag: NonNullable<typeof dragState.current>): HmiMarqueeRect => ({
		...
		const onMouseMove = (moveEvent: MouseEvent) => {
			...
			if (!drag) return;
			onPreviewChange(rectFrom(clientToCanvas(moveEvent.clientX, moveEvent.clientY), drag));
		};
		...
		const onMouseUp = (upEvent: MouseEvent) => {
			...
			if (!drag) return;
			const rect = rectFrom(clientToCanvas(upEvent.clientX, upEvent.clientY), drag);
