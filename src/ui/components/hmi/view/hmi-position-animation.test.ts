...
describe("resolvePositionAnimationOffset", () => {
	it("retourne undefined si le widget ne porte aucune animation de position", () => {
		...
		expect(
			resolvePositionAnimationOffset(widget, () => undefined),
		).toBeUndefined();
	});
	...
	it("résout dx/dy à partir des variables liées, indépendamment l'une de l'autre", () => {
		...
		const getVariableValue = (mnemonic: string) =>
			mnemonic === "X_OFFSET" ? 12 : mnemonic === "Y_OFFSET" ? -5 : undefined;
		expect(resolvePositionAnimationOffset(widget, getVariableValue)).toEqual({
			dx: 12,
			dy: -5,
		});
	});
