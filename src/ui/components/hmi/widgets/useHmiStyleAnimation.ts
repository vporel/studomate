 ...
 * comparé comme `0`/`1`) et retourne ses propriétés. Objet vide si l'animation est absente, la
 * variable introuvable, ou qu'aucune ligne ne correspond — l'appelant retombe alors sur les
...
export default function useHmiStyleAnimation<TProps extends string>(
	...
	if (!animation) return {};
	const entry = Object.values(simulationVariablesStates).find((s) => s.mnemonic === animation.variableMnemonic);
	if (!entry) return {};
	const currentValue = typeof entry.value === "boolean" ? (entry.value ? 1 : 0) : entry.value;
