...
describe("useGridColumns", () => {
	...
	function setup(mode: ProjectMode) {
		...
		return renderHook(() => useGridColumns(["logic-input"]))
	}
	...
	it("defines the mnemonic, type, address and comment columns", () => {
		...
		expect(result.current.map((c) => c.field)).toEqual(["mnemonic", "type", "address", "comment"])
	})
	...
	it("restricts the type column options to the valid types for the given zones", () => {
		...
		expect((typeColumn.valueOptions as { value: string }[]).map((o) => o.value)).toEqual(["BOOL"])
	})
	...
	it("uppercases and trims addresses via valueParser", () => {
		...
		expect(addressColumn.valueParser!(" i0.1 ", undefined, undefined as any, undefined as any)).toBe(
			"I0.1",
		)
		expect(addressColumn.valueParser!("", undefined, undefined as any, undefined as any)).toBe("")
	})

	it("trims mnemonics via valueParser", () => {
		const { result } = setup(ProjectMode.DESIGN)
		const mnemonicColumn = result.current.find((c) => c.field === "mnemonic")!
		expect(mnemonicColumn.valueParser!(" foo ", undefined, undefined as any, undefined as any)).toBe(
			"foo",
		)
	})
})
