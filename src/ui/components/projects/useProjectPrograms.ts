...
const PROGRAM_TYPE_ORDER: Record<ProjectProgram["type"], number> = {
	ladder: 0,
	grafcet: 1,
};
...
export default function useProjectPrograms(): ProjectProgram[] {
	const grafcetsIds = useProjectStore(
		useShallow((state) =>
			state.project ? Object.keys(state.project.grafcets) : [],
		),
	);
	const grafcetsNames = useProjectStore(
		useShallow((state) =>
			state.project
				? Object.values(state.project.grafcets).map((g) => g.name)
				: [],
		),
	);
	const laddersIds = useProjectStore(
		useShallow((state) =>
			state.project ? Object.keys(state.project.ladders) : [],
		),
	);
	const laddersNames = useProjectStore(
		useShallow((state) =>
			state.project
				? Object.values(state.project.ladders).map((l) => l.name)
				: [],
		),
	);
	const laddersRoles = useProjectStore(
		useShallow((state) =>
			state.project
				? Object.values(state.project.ladders).map((l) => l.role)
				: [],
		),
	);
	...
	return useMemo(() => {
		const programs: ProjectProgram[] = [
			...grafcetsIds.map((id, i) => ({
				id,
				name: grafcetsNames[i],
				type: "grafcet" as const,
			})),
			...laddersIds.map((id, i) => ({
				id,
				name: laddersNames[i],
		...
		return programs.sort((a, b) => {
			...
			const roleOrder =
				LADDER_ROLE_ORDER[a.role ?? "standard"] -
				LADDER_ROLE_ORDER[b.role ?? "standard"];
			if (roleOrder !== 0) return roleOrder;
			return a.name.localeCompare(b.name);
		});
	}, [grafcetsIds, grafcetsNames, laddersIds, laddersNames, laddersRoles]);
}
