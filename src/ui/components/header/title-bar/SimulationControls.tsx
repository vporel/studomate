...
const SimulationControls = () => {
	const { mode, simulationMode, simulationPaused, simulationManager } = useProjectStore(
		useShallow((state) => ({
			mode: state.mode,
	...
	if (simulationMode === SimulationMode.CONTINUOUS) {
		return simulationPaused ? (
			<Tooltip title="Reprendre la simulation">
				<IconButton size="small" onClick={() => simulationManager.resumeSimulation()}>
					<PlayArrowRoundedIcon fontSize="small" />
				</IconButton>
			</Tooltip>
		) : (
			<Tooltip title="Mettre en pause">
				<IconButton size="small" onClick={() => simulationManager.pauseSimulation()}>
	...
	if (simulationMode === SimulationMode.STEP_BY_STEP) {
		return (
			<Tooltip title="Avancer d'un cycle">
				<IconButton size="small" onClick={() => simulationManager.stepSimulation()}>
