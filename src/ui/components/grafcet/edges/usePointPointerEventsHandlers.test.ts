...
describe("usePointPointerEventsHandlers", () => {
	...
	function setup(mode: ProjectMode) {
		...
		const setPointsForAdding = jest.fn()
		const { result } = renderHook(() =>
			usePointPointerEventsHandlers(points, setPoints, setPointsForAdding, "edge-1"),
		)
		return { result, setPoints, setPointsForAdding }
	}
	...
	describe("handlePointPointerDown", () => {
		...
		it("deletes an intermediate point on right click", () => {
			const { result } = setup(ProjectMode.DESIGN)
			...
			expect(updater({ points: [[0, 0], [10, 0], [20, 0]] })).toEqual({
				points: [[0, 0], [20, 0]],
			})
		})

		it("does not delete the first or last point on right click", () => {
			const { result } = setup(ProjectMode.DESIGN)
			const event = fakePointerEvent({ buttons: 2 })
	...
	describe("handlePointPointerMove", () => {
		it("updates the moved point's position when the left button is held", () => {
			...
			expect(updater([[0, 0], [10, 0], [20, 0]])).toEqual([[0, 0], [99, 42], [20, 0]])
		})
	...
	describe("handlePointPointerUp", () => {
		it("releases the pointer and persists the points", () => {
			const { result } = setup(ProjectMode.DESIGN)
			const event = fakePointerEvent()
			...
			expect(updateEdgeData).toHaveBeenCalledWith("edge-1", { points: [[0, 0], [10, 0], [20, 0]] })
		})
	})
})
