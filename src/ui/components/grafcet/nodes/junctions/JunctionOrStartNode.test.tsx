...
import JunctionOrStartNode, { JunctionOrStartNodeType } from "./JunctionOrStartNode";
...
describe("JunctionOrStartNode", () => {
	it("se rend sans planter", () => {
		const junction = new JunctionOrStartBuilder().id("junction-1").position(0, 0).build();
		...
		expect(container.querySelector(".junction-or-start-node")).toBeInTheDocument();
	});
});
