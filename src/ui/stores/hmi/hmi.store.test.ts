...
describe("hmi.store", () => {
	describe("addWidget", () => {
		it("ajoute le widget, le sélectionne et active l'annulation", () => {
			...
			expect(Object.values(state.hmiPage.widgets)).toHaveLength(1);
			expect(state.selectedWidgetIds).toEqual([
				Object.values(state.hmiPage.widgets)[0].id,
			]);
	...
	describe("sélection", () => {
		it("toggleWidgetSelection ajoute puis retire un widget de la sélection", () => {
			...
			expect(store.getState().selectedWidgetIds.sort()).toEqual(
				[w1.id, w2.id].sort(),
			);
	...
	describe("updateWidget", () => {
		it("met à jour la position et l'annulation la restaure", () => {
			const store = buildStore();
			store.getState().addWidget("push-button", 10, 20);
			const widgetId = Object.values(store.getState().hmiPage.widgets)[0].id;

			store.getState().updateWidget(widgetId, { position: { x: 100, y: 200 } });

			expect(
				Object.values(store.getState().hmiPage.widgets)[0].position,
			).toEqual({ x: 100, y: 200 });
		...
		it("met à jour les données partiellement sans affecter les autres champs au retour arrière", () => {
			...
			expect(
				Object.values(store.getState().hmiPage.widgets)[0].data,
			).toMatchObject({ label: "Vitesse", min: 10, max: 100 });

			store.getState().commandsStackManager.undoOperation();

			expect(
				Object.values(store.getState().hmiPage.widgets)[0].data,
			).toMatchObject({ label: "Vitesse", min: 0, max: 100 });
		});
	});
	...
	describe("alignSelectedWidgets", () => {
		function twoWidgetsAtDifferentHeights(
			...
			store
				.getState()
				.updateWidget(top.id, { size: { width: 90, height: 40 } });
			store
				.getState()
				.updateWidget(bottom.id, { size: { width: 90, height: 60 } });
			store.getState().selectAllWidgets();
			return { top, bottom };
		}
		...
		function positionOf(
			...
			return store.getState().hmiPage.widgets[widgetId]!.position;
		}
		...
		it("arrondit le résultat à la dizaine la plus proche", () => {
			...
			store
				.getState()
				.updateWidget(top.id, { size: { width: 90, height: 33 } });
			store
				.getState()
				.updateWidget(bottom.id, { size: { width: 90, height: 45 } });
			store.getState().selectAllWidgets();
		...
		function twoWidgetsAtDifferentWidths(store: ReturnType<typeof buildStore>) {
			...
			store
				.getState()
				.updateWidget(left.id, { size: { width: 40, height: 40 } });
			store
				.getState()
				.updateWidget(right.id, { size: { width: 60, height: 40 } });
			store.getState().selectAllWidgets();
			return { left, right };
		}
	...
	describe("removeSelectedWidgets", () => {
		it("retire les widgets sélectionnés et l'annulation les restaure à l'identique, en une seule commande", () => {
			...
			expect(
				Object.values(store.getState().hmiPage.widgets)
					.map((w) => w.id)
					.sort(),
			).toEqual([w1.id, w2.id].sort());
		});
	...
	describe("updateWidget - name", () => {
		it("renomme le widget, et l'annulation restaure le nom précédent", () => {
			...
			expect(Object.values(store.getState().hmiPage.widgets)[0].name).toBe(
				"Départ moteur",
			);
		...
		it("ignore un nom déjà pris par un autre widget de la page", () => {
			...
			expect(store.getState().hmiPage.widgets[w1.id]!.name).toBe(
				"Bouton poussoir_1",
			);
		});
	});
	...
	describe("bringForward / sendBackward", () => {
		...
		it("ne fait rien si le widget est déjà au premier plan", () => {
			...
			store
				.getState()
				.bringForward(Object.values(store.getState().hmiPage.widgets)[1].id);
