...
/**
 * Ces tests n'existent que parce que les factories ne dépendent plus de `ViewManager` :
...
describe("ElementsCommandsFactory (sans instance React Flow)", () => {
	function grafcetWithStep(number: number) {
		return new GrafcetBuilder().addStep(new StepBuilder().id("step-1").number(number).build()).build();
	}
	...
	describe("onNodesRemove", () => {
		it("marque le nœud à supprimer", () => {
			const grafcet = grafcetWithStep(1);
			const existingNodes = NodesFactory.getInitialNodes(grafcet) as GrafcetNodeType[];

			const { commands, nodesIdsToDelete } = ElementsCommandsFactory.onNodesRemove(
				["step-1"],
				grafcet,
				existingNodes,
			);
		...
		it("ignore un identifiant absent de la vue", () => {
			...
			const { nodesIdsToDelete } = ElementsCommandsFactory.onNodesRemove(["inexistant"], grafcet, []);
