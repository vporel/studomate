...
type GrafcetElementEntry = { id: string; element: Element<any>; type: ElementType };

class NodesFactory extends AbstractNodesFactory<GrafcetNodeType, Grafcet, GrafcetElementEntry> {
	protected getDomainElements(grafcet: Grafcet): GrafcetElementEntry[] {
		...
		(Object.keys(typesToElementsMap) as ElementType[]).forEach((type) => {
			typesToElementsMap[type].forEach((element) => allElements.push({ id: element.id, element, type }));
		});
		return allElements;
	}
