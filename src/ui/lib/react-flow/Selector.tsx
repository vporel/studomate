...
const Selector = ({
	...
	const rectanglePoints = React.useRef<{ p1: XYPosition; p2: XYPosition }>({
		p1: { x: 0, y: 0 },
		p2: { x: 0, y: 0 },
	});
	...
	const selectorStyle: SelectorStyle = {
		fill: style?.fill ?? "rgba(0, 89, 220, 0.08)",
		stroke: style?.stroke ?? "rgba(0, 89, 220, 0.8)",
		rectangle: {
			rightLeft: {
				fill: "rgba(0, 220, 89, 0.08)",
				stroke: "rgba(0, 220, 89, 0.8)",
				...style?.rectangle?.rightLeft,
			},
		},
	};
