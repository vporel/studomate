...
describe("initialConnectionPoints", () => {
	it("aucun point sur la même ligne", () => {
		expect(
			initialConnectionPoints({ row: 0, col: 0 }, { row: 0, col: 3 }),
		).toEqual([]);
	});
...
describe("pushConnectionBend", () => {
	...
	it("aucun effet si points est vide (même ligne mémorisée)", () => {
		expect(
			pushConnectionBend([], "source", { row: 0, col: 1 }, { row: 2, col: 3 }),
		).toEqual([]);
	});

	it("la source s'éloigne du coude : le stub s'allonge, le coude ne bouge pas", () => {
		const result = pushConnectionBend(
			points,
			"source",
			{ row: 0, col: 0 },
			{ row: 2, col: 3 },
...
describe("findCellCrossings", () => {
	...
	it("left/right : une connexion inter-lignes longe des cellules vides sans les traverser", () => {
		...
		const connection = new Connection(
			"c1",
			{ id: contactTop.id, type: "contact", handle: "source" },
			{ id: coilBottom.id, type: "coil", handle: "target" },
		);
		section.connections = [connection];
	...
	it("left et right peuvent venir de deux connexions différentes sur la même cellule", () => {
		...
		const leftTouchConnection = new Connection(
			"c-left",
			{ id: contactLeft.id, type: "contact", handle: "source" },
			{ id: targetOfLeft.id, type: "coil", handle: "target" },
		);
		const rightTouchConnection = new Connection(
			"c-right",
			{ id: contactRight.id, type: "contact", handle: "source" },
			{ id: targetOfRight.id, type: "coil", handle: "target" },
		);
