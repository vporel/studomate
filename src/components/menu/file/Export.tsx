...
import { GrafcetPageData, usePagesContext } from "@/PagesContext";
import { getNodesBounds } from "@xyflow/react";
import domToImage from "dom-to-image"
 
export function DownloadButton({pageId}: {pageId: string}) {
	const {getPageData} = usePagesContext()
	const onClick = () => {
		const pageData = getPageData(pageId) as GrafcetPageData
		...
		const htmlElement = document.querySelector('.grafcet-page#'+pageId+' .react-flow__viewport') as HTMLElement
		domToImage.toJpeg(htmlElement, {
	...
	return (
		<button onClick={onClick}>Télécharger</button>
	);
}