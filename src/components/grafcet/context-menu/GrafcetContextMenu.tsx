...
import { GrafcetContextMenuElement, GrafcetcontextMenuItemType, GrafcetContextMenuProps } from "./grafcet-context-menu-types"
...
const GrafcetContextMenu = () => {
	...
	const menuItems: GrafcetcontextMenuItemType[][] = useMemo(() => {
		...
		if(element.type == "pane"){
			...
			return [...junctionContextMenuItems(element as JunctionNode, contextMenuEvents), ...commonNodesItems]
		}else{
	...
	return (visible && menuItems.length > 0) && <Box 
		...
		component="ul"
		sx={{
			position: "absolute", left: position.x, top: position.y, zIndex: 10,
			...
			".sub-items-container": {
				...
				background: "white", minWidth: "160px", minHeight: "20px",
	...
	>
		{menuItems.map((group, index) => <Fragment key={index}>
			{group.map(item => <MenuItem 
				...
				className={`item`}
			>
				<Box component="span" className="label">{item.label}</Box>
				<Box component="span" className="right-text">{(item.subItems && item.subItems.length > 0) ? <ChevronRightIcon /> : item.shortcut}</Box>
				{item.subItems && <Box className="sub-items-container" component="ul">
					{item.subItems.map(subItem => <MenuItem 
						...
						className={`sub-item`}
					>
						<Box component="span" className="label">{subItem.label}</Box>
						<Box component="span" className="right-text">{subItem.shortcut}</Box>
					</MenuItem>)}
				</Box>}
			</MenuItem>)}
