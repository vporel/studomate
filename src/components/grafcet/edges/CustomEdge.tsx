...
function getPointsForAdding(points: [number, number][]): [number, number][]{
	...
	for(let i = 1; i < points.length; i++) pointsForAdding.push([(points[i][0]+points[i-1][0])/2, (points[i][1]+points[i-1][1])/2])
...
const CustomEdge = ({
  ...
  selected
	...
	return <Box component="g" sx={{
		...
		{points.map((p, index) => (index != 0 && index != points.length-1) && <g key={index}>
			<circle className='react-flow__edge-path__point' key={index} cx={p[0]} cy={p[1]} r={3} fill={color} stroke={color} strokeWidth={1.5}/>
			<circle
		...
		{pointsForAdding.map((p, index) => <g key={index}>
			<circle className='react-flow__edge-path__point' cx={p[0]} cy={p[1]} r={3} fill="#fff" stroke={color} strokeWidth={1.5}/>
			<circle
				className='react-flow__edge-path__point'
				cx={p[0]}
				cy={p[1]}
