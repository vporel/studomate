...
export const nodesSchemasClasses: Record<NodeTypeKey, any> = {
	step: Step,
	action: Action,
	transition: Transition,
	"step-referral-source": StepReferral,
	"step-referral-target": StepReferral,
...
export function validateConnection(
	...
	const sourceType = nodes.find((n) => n.id == connection.source)!
		.type as NodeTypeKey;
	const targetType = nodes.find((n) => n.id == connection.target)!
		.type as NodeTypeKey;
	...
	if (
		...
		return false;
	if (
		sourceType == "junction-or-start" &&
		!["transition"].includes(targetType)
	)
		return false;
	if (sourceType == "junction-or-end" && !["step"].includes(targetType))
		return false;
	if (sourceType == "junction-and-start" && !["step"].includes(targetType))
		return false;
	if (
		sourceType == "junction-and-end" &&
		!["transition"].includes(targetType)
	)
		return false;
	if (sourceType == "step-referral-target" && !["step"].includes(targetType))
		return false;
	return true;
}
