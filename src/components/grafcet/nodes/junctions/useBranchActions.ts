...
export default function useBranchActions(nodeId: string, nodeData: JunctionNodeData): {add: (buttonIndex: number) => void}{
	...
	const add = useCallback((buttonIndex: number) => {
		...
		if(nodeData.branchesPositions.length == 0) newBranchPosition = nodeData.width/2
			...
			else if(buttonIndex == nodeData.branchesPositions.length) newBranchPosition = (nodeData.branchesPositions[nodeData.branchesPositions.length-1] + nodeData.width)/2
			else{
				newBranchPosition = (nodeData.branchesPositions[buttonIndex-1] + nodeData.branchesPositions[buttonIndex])/2
			}
		}
