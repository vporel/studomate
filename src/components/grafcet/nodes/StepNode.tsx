...
export const STEP_NODE_DEFAULT_DIMENSIONS: Dimensions = {width: 40, height: 40}
...
export const STEP_NODE_DEFAULT_DATA: StepNodeData = {
	number: "",
	isInitial: false
}

export type StepNodeType = Node<StepNodeData> & {type: "step"}
...
const StepNode: FC<StepNodeProps> = ({id, data, selected}) =>{
	...
	return <>
		...
		<HandleWithConnectionsLimit
			...
			style={{borderColor: borderColor, backgroundColor: borderColor}}
		/>
		<Box className="grafcet-node step-node" sx={{
			width: STEP_NODE_DEFAULT_DIMENSIONS.width+"px", 
			height: STEP_NODE_DEFAULT_DIMENSIONS.height+"px", 
			...
			<input 
				...
				onChange={e => updateNodeData(id, 
					{
						...data, 
						number: (e.target.value == "" || parseInt(e.target.value) < 0) ? "" : parseInt(e.target.value)
					}
				)} 
				style={{
