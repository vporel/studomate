...
describe("HmiPage", () => {
	...
	describe("addWidget / removeWidget", () => {
		it("ajoute un widget et le retire par id", () => {
			...
			expect(page.widgets[0].id).toBe(w2.id);
		});
	...
	describe("updateWidget", () => {
		it("met à jour la position", () => {
			...
			expect(page.widgets.find((w) => w.id === w1.id)!.position).toEqual({ x: 50, y: 60 });
		});
		...
		it("met à jour la taille", () => {
			...
			expect(page.widgets.find((w) => w.id === w1.id)!.size).toEqual({ width: 90, height: 100 });
		});
		...
		it("met à jour les données partiellement", () => {
			...
			const updated = page.widgets.find((w) => w.id === w1.id)!;
		...
		it("ignore un id inexistant", () => {
			...
			expect(() => page.updateWidget("inconnu", { position: { x: 0, y: 0 } })).not.toThrow();
		});
	});
