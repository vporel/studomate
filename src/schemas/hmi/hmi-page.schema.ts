 ...
 * widget par son id, jamais par position, et l'ordre du tableau n'a aucun sens — chaque widget
 * porte sa position `x/y`, et un champ `stackOrder` gère la superposition. Même forme que
 * `Project.programs` / `Grafcet` (collections d'éléments).
 */
export default class HmiPage {
	id: string;
	name: string;
	widgets: Record<string, HmiWidget>;
	...
	updateWidget(
		...
		if (partial.position !== undefined)
			widget.position = { ...partial.position };
		if (partial.size !== undefined) widget.size = { ...partial.size };
		if (partial.stackOrder !== undefined)
			widget.stackOrder = partial.stackOrder;
		...
		if (partial.data !== undefined)
			widget.data = { ...widget.data, ...partial.data } as typeof widget.data;
	}
	...
	copy(): HmiPage {
		...
		copy.widgets = Object.fromEntries(
			Object.entries(this.widgets).map(([id, w]) => [id, w.copy()]),
		);
		return copy;
	}
	...
	static createFromJSON(json: string): HmiPage {
		...
		page.widgets = Object.fromEntries(
			Object.entries(raw.widgets ?? {}).map(([id, w]) => [
				id,
				HmiWidget.createFromJSON(JSON.stringify(w)),
			]),
		);
		return page;
	}
