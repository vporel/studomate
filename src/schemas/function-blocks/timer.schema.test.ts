...
describe("createTimerBlockVariables", () => {
	it("génère les variables IN/Q (BOOL) et ET (TIME), rattachées au bloc", () => {
		...
		expect(variables.map((v) => v.mnemonic)).toEqual(["Tempo1.IN", "Tempo1.Q", "Tempo1.ET"]);
...
describe("createTimerBlockElement", () => {
	it("pose un bloc timer à la position donnée, avec sa config dans data.params", () => {
		const block = createTimerBlockElement({ name: "Tempo1", timerType: "TON", pt: "T#5s" }, 2, 3);
	...
	it("chaque bloc créé a un id distinct", () => {
		const a = createTimerBlockElement({ name: "Tempo1", timerType: "TON", pt: "T#5s" }, 0, 0);
		const b = createTimerBlockElement({ name: "Tempo1", timerType: "TON", pt: "T#5s" }, 0, 0);
...
describe("getTimerBlockParams", () => {
	it("renvoie la config d'un bloc timer", () => {
		const block = createTimerBlockElement({ name: "Tempo1", timerType: "TOF", pt: "T#1s", et: "Sortie" }, 0, 0);

		expect(getTimerBlockParams(block)).toEqual({ name: "Tempo1", timerType: "TOF", pt: "T#1s", et: "Sortie" });
	});

	it("renvoie null pour un bloc d'un autre type", () => {
		const block = createTimerBlockElement({ name: "Tempo1", timerType: "TON", pt: "T#1s" }, 0, 0);
