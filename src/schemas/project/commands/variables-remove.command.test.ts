...
describe("VariablesRemoveCommand", () => {
	...
	it("retire les variables de la charge utile, et l'annulation les restaure", () => {
		...
		expect(project.variables.find((v) => v.id === "v1")).toMatchObject({ address: "%I0.0", comment: "capteur" });
	});
	...
	it("round-trip execute→cancel laisse le projet inchangé", () => {
		...
		expect(JSON.parse(JSON.stringify(project)).variables.sort((a: any, b: any) => a.id.localeCompare(b.id))).toEqual(
			JSON.parse(before).variables.sort((a: any, b: any) => a.id.localeCompare(b.id)),
		);
	});
});
