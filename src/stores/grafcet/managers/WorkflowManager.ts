...
import GrafcetConnectionsCommandsFactory from "../factories/GrafcetConnectionsCommandsFactory.class";
import GrafcetElementsCommandsFactory from "../factories/GrafcetElementsCommandsFactory.class";
...
export default class WorkflowManager {
	...
	handleNodesChange(changes: NodeChange<GrafcetNodeType>[]): void {
		...
		const { commands: edgesCommands, edgesDataToApply: edgesDataToUpdate } =
			GrafcetConnectionsCommandsFactory.onNodesMovedOrResized(
	...
	updateNodeData(
		...
		const grafcet = this.getStoreState().grafcet;
		viewManager.throwErrorIfNotReady();
		const { commands, nodeDataToUpdate } = GrafcetElementsCommandsFactory.onNodeDataChange(
			nodeId,
			newData,
			grafcet,
			viewManager,
	...
	updateEdgeData(
		...
		const grafcet = this.getStoreState().grafcet;
		viewManager.throwErrorIfNotReady();
		const { commands, edgeDataToApply: newDataToApply } =
			GrafcetConnectionsCommandsFactory.onEdgeDataChange(
	...
	addNodesAndEdges(newNodes: GrafcetNodeType[], newEdges: GrafcetEdgeType[]): void {
		...
		const viewManager = this.getStoreState().viewManager;
		viewManager.throwErrorIfNotReady();
		const { commands: nodesCommands, nodesToAdd } = GrafcetElementsCommandsFactory.onNodesAdd(
			newNodes,
			grafcet,
			viewManager,
		);
		const { commands: edgesCommands, edgesToAdd } = GrafcetConnectionsCommandsFactory.onEdgesAdd(
			newEdges,
			nodesToAdd,
			grafcet,
			viewManager,
	...
	deleteNodesAndEdges(nodesIds: string[], edgesIds: string[]): void {
		...
		} = GrafcetElementsCommandsFactory.onNodesRemove(nodesIds, grafcet, viewManager);
		const { commands: commandsFromEdges, edgesIdsToDelete: list2EdgesIdsToDelete } =
			GrafcetConnectionsCommandsFactory.onEdgesRemove(
